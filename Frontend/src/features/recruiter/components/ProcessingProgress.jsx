...
export default function ProcessingProgress({ analysisId, onProgress }) {
  ...
  return <section aria-label="Resume processing" className="mt-6 rounded-2xl border border-slate-200 p-5 dark:border-white/10">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-semibold">Resume processing</h3><p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{job.completed}/{job.total} complete · {job.failed} failed · {job.queued} queued · {job.processing} processing</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${done ? (job.failed ? 'bg-amber-500/10 text-amber-800 dark:text-amber-300' : 'bg-emerald-500/10 text-emerald-800 dark:text-emerald-300') : 'bg-blue-500/10 text-blue-800 dark:text-blue-300'}`}>{job.status}</span></div>
    ...
    <p className="mt-2 break-all text-xs text-slate-500 dark:text-slate-400">Job ID: {job.id}. Complete includes successful and failed files.</p>
