...
export default function ResumeUploads({ analysisId }) {
  ...
  const upload = async () => {
    ...
    try {
      ...
      setMessage(`${records.length} resume${records.length === 1 ? '' : 's'} uploaded. Processing has not started.`);
  ...
  return <section aria-label="Analysis resumes" className="mt-8 rounded-2xl border border-slate-200 p-5 dark:border-white/10">
    ...
    <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">Upload 1–10 PDFs at a time, up to 5 MB each. Files are stored privately; parsing and matching have not started.</p>
